...
def calculate_tax_by_tax_slab(
	...
	tax_amount, other_taxes = calculate_other_charges(
		tax_amount,
		annual_taxable_earning,
		tax_slab,
	)
...
def calculate_base_tax_from_tax_slabs(
	annual_taxable_earning, tax_slab, eval_globals, eval_locals
):
	tax_amount = 0
	...
	for slab in tax_slab.slabs:
		...
		if cond and not eval_tax_slab_condition(
			cond,
			eval_globals,
			eval_locals,
		):
		...
		if not slab.to_amount and annual_taxable_earning >= slab.from_amount:
			tax_amount += (
				annual_taxable_earning - slab.from_amount + 1
			) * slab.percent_deduction * 0.01
			continue

		if (
			annual_taxable_earning >= slab.from_amount
			and annual_taxable_earning < slab.to_amount
		):
			tax_amount += (
				annual_taxable_earning - slab.from_amount + 1
			) * slab.percent_deduction * 0.01
			...
			tax_amount += (
				slab.to_amount - slab.from_amount + 1
			) * slab.percent_deduction * 0.01

	return tax_amount


def calculate_other_charges(
	tax_amount,
	annual_taxable_earning,
	tax_slab,
):
...
def eval_tax_slab_condition(
	...
	try:
		...
		frappe.throw(
			_("Syntax error in condition: {0} in Income Tax Slab").format(str(err))
		)

	except Exception as e:
		frappe.throw(
			_("Error in formula or condition: {0} in Income Tax Slab").format(str(e))
		)
		raise
